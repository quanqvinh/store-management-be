...
export class LocalAdminStrategy extends PassportStrategy(Strategy, 'local-admin') {
	...
	async validate(username: string, password: string) {
		if (!Joi.string().pattern(Pattern.username.normal).pattern(Pattern.username.hasLetter))
			throw new FailedLoginException(IdentifierType.USERNAME)
		const employee = await this.authService.validateEmployee(username, EmployeeRole.ADMIN, password)
